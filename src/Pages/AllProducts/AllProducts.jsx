 ...
 if (location.state) {
  ...
  const startListeningInstance = () => {
    ...
    if (!SpeechRecognition) {
      Swal.fire({
        icon: "error",
        title: "Oops",
        text: "Speech engine not supported in browser.",
        background: "#0c0d14",
        color: "#fff",
      });
  ...
  const applyAiRecommendation = () => {
    if (aiSelectedCategories.length === 0) {
      Swal.fire({
        text: "Select at least 1 category for AI analytics.",
        icon: "warning",
        background: "#0c0d14",
        color: "#fff",
      });
    ...
    <div className="bg-[#050508] text-white min-h-screen overflow-x-clip relative select-none pb-32">
      ...
      <div className="max-w-7xl mx-auto pt-24 pb-4 px-6 md:px-12 text-center space-y-4">
        ...
        <h1 className="text-4xl md:text-7xl font-extrabold italic tracking-tighter uppercase">
          All{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#7c74ff] to-purple-400">
            Products
          </span>{" "}
          GRID
        </h1>
      ...
      <div className="sticky top-24 z-10 max-w-6xl mx-auto px-4 md:px-6 my-8">
        <div className="bg-[#0d0f17]/80 border border-gray-900/90 rounded-[24px] p-2.5 backdrop-blur-xl shadow-[0_0_40px_rgba(0,0,0,0.8)] flex items-center justify-between gap-4">
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar w-full py-1">
            {categories.map((cat) => {
              const isActive =
                selectedCategory === cat.name && !aiActiveFilters;
      ...
      <div className="max-w-6xl mx-auto px-6 mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex items-center gap-3 bg-[#11131c]/40 border border-gray-900/80 focus-within:border-[#5046e5]/50 px-5 py-3.5 rounded-[20px] backdrop-blur-md transition-all duration-300 shadow-inner w-full md:max-w-xl">
          ...
          <div
            ...
            className="cursor-pointer p-1.5 rounded-lg bg-indigo-600/10 hover:bg-indigo-600/30 transition"
          >
        ...
        <div className="flex items-center gap-2 bg-[#11131c]/40 border border-gray-900/80 px-4 py-3 rounded-[20px] backdrop-blur-md w-full md:w-auto shrink-0">
          ...
          >
            <option value="default" className="bg-[#0c0d14] text-white">
              Default Matrix
            </option>

            <option value="lowToHigh" className="bg-[#0c0d14] text-white">
              Price: Low to High
            </option>

            <option value="highToLow" className="bg-[#0c0d14] text-white">
              Price: High to Low
            </option>
          </select>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 mb-12">
        ...
        >
          <div className="flex items-center gap-3 text-left">
            <div
              className={`p-2.5 rounded-xl ${aiActiveFilters ? "bg-purple-500 text-white animate-pulse" : "bg-purple-500/10 text-purple-400"}`}
            >
            ...
            <div>
              <h4 className="text-sm font-black italic uppercase tracking-wider">
                AI Synapse Multi-Category Engine
              </h4>

              <p className="text-[11px] text-gray-500 mt-0.5">
                {aiActiveFilters
                  ? `Optimized Matrix Enabled for: ${aiActiveFilters.categories.join(", ")} (Low Price + High Rating)`
                  : "Tap to select multiple categories using checkboxes and filter best deals."}
              </p>
            </div>
          </div>

          <span
            className={`text-[10px] font-black tracking-widest px-3 py-1.5 rounded-lg border uppercase shrink-0 ${
              aiActiveFilters
                ? "bg-purple-500/20 border-purple-400 text-purple-300"
                : "bg-black border-gray-800 text-gray-500 group-hover:text-purple-400"
            }`}
          >
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        {filteredProducts.length > 0 ? (
            ...
            {filteredProducts.map((product) => (
              ...
              >
                  ...
                  <div className="aspect-square w-full relative overflow-hidden rounded-[20px] bg-black/60 border border-gray-950">
                    ...
                    <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                      ...
                      <Link to={`/details/${product._id || product.id}`}>
                        <button className="p-2 bg-black/80 hover:bg-indigo-600/20 rounded-xl border border-gray-800 hover:border-indigo-500 text-gray-400 hover:text-indigo-400 transition backdrop-blur-sm">
                ...
                <div className="pt-4 mt-4 border-t border-gray-900/40 flex items-center justify-between px-1">
                  <div>
                    <p className="text-[8px] font-black italic text-gray-600 uppercase tracking-widest">
                      NET VAL
                    </p>
          ...
          <div className="text-center py-24 bg-[#0d0f17]/30 border border-gray-900 rounded-[28px]">
            <p className="text-gray-500 font-black italic tracking-widest uppercase text-xs">
              No Matrix Matches Your Query
            </p>
          </div>
        )}
      </div>
          ...
          <div className="bg-[#0b0c16] border border-gray-900 w-full max-w-lg rounded-[32px] p-6 relative shadow-[0_0_55px_rgba(80,70,229,0.15)] space-y-6">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-gray-500">
                System is capture processing your vocal query inputs...
              </p>
            </div>
            ...
            <div className="flex items-center justify-center gap-1.5 h-16">
              {isListening ? (
                ...
                <div className="w-full text-center text-xs font-mono text-red-500 uppercase tracking-widest">
                  [ Core Listening Paused ]
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                onClick={() => {
                  stopListeningInstance();
                  setVoiceText("");
                  setTimeout(() => startListeningInstance(), 200);
                }}
      ...
      {isAiModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-[#0b0c16] border border-gray-900 w-full max-w-xl rounded-[32px] p-6 relative shadow-[0_0_55px_rgba(139,92,246,0.15)] space-y-6">
            ...
            <div className="text-center space-y-1">
              <h3 className="text-lg font-black uppercase tracking-wider text-purple-400 italic">
                Select Synapse Segments
              </h3>

              <p className="text-xs text-gray-500">
                Pick multiple clusters to trigger optimized budget/rate sorting
              </p>
            </div>
