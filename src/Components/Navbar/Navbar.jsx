    ...
    <>
      <nav className="bg-[#09090d] dark:bg-[#09090d] bg-white text-gray-800 dark:text-white px-10 py-4 flex items-center justify-between border-b border-gray-200 dark:border-gray-900 sticky top-0 z-50 transition-colors duration-300">
        ...
        <div className="hidden lg:flex items-center space-x-6">
          <button
            ...
            className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition duration-200"
          >
          ...
          <div>
            {loading ? (
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-gray-950 overflow-hidden z-50">
                <div
                ...
                {user ? (
                  <div className="hidden lg:flex items-center gap-4 relative">
                    ...
                    <div
                      ...
                      className="flex items-center gap-2.5 bg-black/40 border border-gray-900 hover:border-purple-500/30 px-3 py-1.5 rounded-xl cursor-pointer transition-all duration-300 group select-none"
                    >
                   
                      <div className="w-8 h-8 rounded-full border border-purple-500/30 overflow-hidden p-0.5 shadow-[0_0_10px_rgba(168,85,247,0.2)]">
                        <img
                          src={
                      ...
                      <div className="flex items-center gap-1">
                        <span className="text-xs font-mono font-bold text-gray-300 group-hover:text-white transition-colors">
                          {user?.displayName || "Active Node"}
                        </span>
                        <ChevronDown
                          className={`w-3.5 h-3.5 text-gray-500 group-hover:text-purple-400 transition-transform duration-300 ${isDropdownOpen ? "rotate-180" : ""}`}
                        />
                      </div>
                    </div>
                    ...
                    {isDropdownOpen && (
                      <>
                        ...
                        <div className="absolute top-full right-0 mt-2 w-48 bg-[#08090e]/95 border border-gray-900 rounded-xl p-1.5 shadow-[0_10px_30px_rgba(0,0,0,0.7)] backdrop-blur-xl z-50 animate-fadeIn">
                          <Link
                            to="/dashboard"
                            onClick={() => setIsDropdownOpen(false)}
                            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg font-mono text-[11px] font-bold text-gray-400 hover:text-white hover:bg-white/[0.02] border border-transparent hover:border-indigo-500/20 transition-all duration-200"
                          >
                    ...
                    <button
                      onClick={Signout}
                      className="items-center group relative bg-indigo-600 text-white px-8 py-2 rounded-[2rem] font-bold text-lg hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-600/30 overflow-hidden"
                    >
                      SignOut
                    </button>
                  </div>
                  ...
                  <Link to={"/login"}>
                    <button className="hidden lg:flex items-center group relative bg-indigo-600 text-white px-8 py-2 rounded-[2rem] font-bold text-lg hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-600/30 overflow-hidden">
                      Login
                    </button>
                  </Link>
                )}
      ...
      {isVoiceModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-[#090b11]/80 backdrop-blur-md"
          ...
          <div className="relative bg-[#0d111b] border border-gray-800/80 w-full max-w-[440px] rounded-[36px] p-8 shadow-2xl flex flex-col items-center text-center animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="w-full flex justify-end relative z-10">
              <button
                onClick={closeVoiceModal}
                className="p-2 bg-gray-800/50 hover:bg-gray-800 text-gray-400 hover:text-white rounded-full transition"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="mt-2 mb-6 relative flex items-center justify-center">
              ...
              >
                <Mic
                  className={`w-7 h-7 ${isListening ? "text-white" : "text-indigo-400"}`}
                />
              </button>
            </div>
            ...
            <div className="w-full bg-[#111622]/40 border-2 border-dashed border-gray-800/80 rounded-[24px] p-6 min-h-[110px] flex flex-col items-center justify-center mb-6 relative">
              <p className="text-lg font-bold italic text-gray-300 px-2 line-clamp-3">
                {transcript ||
                  (isListening ? "Say something..." : "Click mic to speak")}
              </p>
